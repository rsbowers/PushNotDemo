...
device_provider = new DeviceProvider(server_config , function(error, connected){
 ...
 device_provider.setIndex('devices', {_id:1}, function(error, index){
...
console.log('Server running at http://54.184.86.146');
...
function registerDevice(request, response) {
 ...
 request.on('data', function(data) {
  ...
  device_provider.save('devices', device, function(error, doc){
   if(error) throw error
...
function notifyDevices(data) {
 ...
 device_provider.findAll('devices', function(error, doc){
  if(error) throw error
  if(doc != null){
   console.log(doc);
   var token = doc.token;
